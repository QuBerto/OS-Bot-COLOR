...
class OSRSWoodcutter(OSRSBot):
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            ...
            if not self.mouseover_text(contains="Steal", color=clr.OFF_WHITE) and not self.__move_mouse_to_nearest_tree():
                failed_searches += 1
            ...
            if not self.mouseover_text(contains="Steal", color=clr.OFF_WHITE):
                continue
            self.mouse.click()
            time.sleep(0.5)
    ...
    def __move_mouse_to_nearest_tree(self, next_nearest=False):
        ...
        """
        trees = self.get_all_tagged_in_rect(self.win.game_view, clr.YELLOW)
        tree = None
        if not trees:
            return False
        ...
        if next_nearest:
            self.mouse.move_to(tree.random_point(), mouseSpeed="fastest", knotsCount=2)
        else:
    ...
    def __drop_logs(self, api_s: StatusSocket):
        ...
        """
        slots = api_s.get_inv_item_indices(
            [
                ids.BANANA,
                ids.PINEAPPLE,
                ids.REDBERRIES,
                ...
                ids.COOKING_APPLE,
                ids.LIME,
                ids.GOLOVANOVA_FRUIT_TOP,
                ids.JANGERBERRIES,
            ]
        )
        if len(slots) == 0 and api_s.get_is_inv_full():
            self.update_progress(1)
            self.__logout("Finished.")
            self.stop()
        self.drop(slots)
        self.logs += len(slots)
        self.log_msg(f"Items stolen: ~{self.logs}")
        time.sleep(1)
