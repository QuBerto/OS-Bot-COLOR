...
class OSRSShilloFisher(OSRSBot):
    def __init__(self):
        ...
        for option in options:
            if option == "running_time":
                ...
                if self.Input == ["Remote"]:
                    self.mouse.RemoteInputEnabledSet = True
                elif self.Input == ["PAG"]:
                    self.mouse.RemoteInputEnabledSet = False
                else:
                    self.log_msg("Failed to set mouse")
            else:
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            trees = self.find_color(self.win.game_view, "custom_cyan")
            print(trees)
            ...
            if not self.mouseover_text(contains="Lure", color=clr.OFF_WHITE) and not self.__move_mouse_to_nearest_tree():
                failed_searches += 1
    ...
    def __move_mouse_to_nearest_tree(self, next_nearest=False):
        ...
        """
        trees = self.find_color(self.win.game_view, "custom_purple")
        print(trees)
        tree = None
        if not trees:
            walker = Walker(self)
            walker.walk_to((2850, 2970))
            return False
    ...
    def __bank(self, api_s: StatusSocket):
        """
        Private function for dropping logs. This code is used in multiple places, so it's been abstracted.
        Since we made the `api` and `logs` variables assigned to `self`, we can access them from this function.
        """
        walker = Walker(self)
        walker.walk_to(loc.SHILO_VILLAGE)
