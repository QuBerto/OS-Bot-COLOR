...
class OSRSAltar(OSRSBot):
    ...
    def save_options(self, options: dict):
        ...
        for option in options:
            if option == "running_time":
                ...
                if "Colossal_pouch" in options[option] and not self.is_only_value_in_list(options[option], "Colossal_pouch"):
    ...
    def is_only_value_in_list(self, lst, value):
        ...
        and False otherwise.
        """
        return len(lst) == 1 and lst[0] == value
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            ...
            if (
                ...
                and self.api_m.get_if_item_in_inv(ids.COSMIC_RUNE)
                and self.round_since > self.repair_after
                and self.pouch != "no"
                or self.repair_now
            ):
                self.round_since = 0
    ...
    def open_inventory(self):
        if self.invetory_open is False:
    ...
    def get_position_pouch(self, pouch):
        pouch_img = imsearch.BOT_IMAGES.joinpath("items", pouch + ".png")
        ...
        if self.get_hp() < self.panic_stop:
            while self.get_hp() < self.api_m.get_skill_level("Hitpoints") * 0.8:
                ...
                if food_location:
                    ...
                    food = False
                    while not food:
                        food_img = imsearch.BOT_IMAGES.joinpath("items", "Lobster.png")
    ...
    def do_single_dialogue(self, dialogue_num):
        dialogue_img = imsearch.BOT_IMAGES.joinpath("scraper", "dialogue_" + dialogue_num + ".png")
    ...
    def click_pouch(self, type_pouch, inventory_change=False, empty_fill="Empty", insane_mode=False):
        ...
        elif insane_mode and empty_fill == "Fill" and self.bank_position_essence:
            self.mouse.move_to(self.bank_position_essence.random_point(), mouseSpeed=self.mouse_speed)
    ...
    def click_altar(self, first=False, insane_mode=False):
        ...
        while not self.mouseover_text("Craft"):
            ...
            if first:
                ...
                craft_runes_img = imsearch.BOT_IMAGES.joinpath("scraper", "craft_rune.png")
        ...
        while current_inv_qt == len(self.api_m.get_inv()):
            if first:
                if not self.invetory_open and rd.random_chance(probability=0.01):
        ...
        if first and not self.invetory_open:
            self.open_inventory()
            if insane_mode and "Colossal_pouch" in self.pouch:
                self.mouse.move_to(self.pouch_position["Colossal_pouch"].random_point(), mouseSpeed=self.mouse_speed)
        return True
    ...
    def click_ladder(self):
        ...
        while ladder_not_found:
            ...
            if not self.mouseover_text("Climb", color=clr.OFF_WHITE):
                ...
                climb_ladder_img = imsearch.BOT_IMAGES.joinpath("scraper", "climb_ladder.png")
            ...
            while not self.api_m.get_is_player_idle():
                time.sleep(3 / 10)
                if not self.invetory_open and rd.random_chance(probability=0.05):
                    # Click the inventory tab
                    self.open_inventory()
                    if self.last_banker:
